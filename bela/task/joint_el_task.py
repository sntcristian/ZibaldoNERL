...
class JointELTask(LightningModule):
    def __init__(
            ...
            optim: OptimConf,
            embedding_dim: int,  # New parameter to specify embedding dimension
            embeddings_path: Optional[str] = None,
            faiss_index_path: Optional[str] = None, # Keep for backwards compatibility, but make optional
        ...
        self.embedding_dim = embedding_dim  # Now required parameter
    ...
    def setup_gpu_index(self):
        """Setup GPU index by converting the loaded CPU index"""
        # Load FAISS index instead of embeddings
        # logger.info(f"Loading FAISS index from {self.faiss_index_path}")
        self.faiss_index = faiss.read_index(self.faiss_index_path)

        # Verify the index dimension matches the expected embedding dimension
        if self.faiss_index.d != self.embedding_dim:
            raise ValueError(
                f"FAISS index dimension ({self.faiss_index.d}) does not match "
    ...
    def setup(self, stage: str):
        ...
        # Setup GPU index if requested
        if self.use_gpu_index:
            logger.info(f"Converting index to GPU")
            self.setup_gpu_index()
        else:
            logger.info(f"Using CPU index")
